...
import { BooksStoreConstants } from './books.constants';

export const initialState: BooksState = {
  books: JSON.parse(
    localStorage.getItem(BooksStoreConstants.STORE_ACCESSORS.BOOKS) ||
      BooksStoreConstants.DEFAULT_VALUES.JSONED_BOOKS_ARRAY
  ),
};

export const booksReducers = createReducer(
  initialState,
  on(BooksActions.addBook, (state: BooksState, action: BookDto): BooksState => {
    return {
      ...state,
  ...
  on(
    ...
    (
      state: BooksState,
      action: BooksActions.DeleteBookActionProps
    ): BooksState => {
  ...
  on(
    ...
    (
      state: BooksState,
      action: BooksActions.EditBookActionProps
    ): BooksState => {
      const newState: BooksState = JSON.parse(JSON.stringify(state));
