...
export class ManageBookDialogComponent implements OnInit {
  ...
  readonly YEAR_CONTROL_ACCESSOR = 'year';
  ...
  public get yearFormValue(): Date {
    return this.formGroup.get(this.YEAR_CONTROL_ACCESSOR)?.value;
  }
  ...
  public handleYearChoice(year: Date, picker: MatDatepicker<Date>): void {
    this.formGroup
      .get(this.YEAR_CONTROL_ACCESSOR)
      ?.setValue(year.getFullYear());
    picker.close();
  }
  ...
  public addBook(): void {
    if (this.formGroup.invalid) {
      ...
      this.notifyService.showMessage(
        this.ADDED_SUCCESSFULLY_MESSAGE,
  ...
  public saveEditingChanges(): void {
    if (this.formGroup.invalid) {
      ...
      this.booksManagerService.saveEditingChanges(
        this.buildBookDto(),
        this.data!.index
      );
      this.dialogRef.close();
      this.notifyService.showMessage(
        this.EDITED_SUCCESSFULLY_MESSAGE,
        NotifyServiceConstants.MESSAGE_TYPES.SUCCESS
      );
    }
  }
  ...
  private buildForms(): void {
    this.formGroup = new FormGroup({
      author: new FormControl<string>(
        this.data ? this.data.book.author : '',
        this.customRequiredValidator
      ),
      bookName: new FormControl<string>(
        this.data ? this.data.book.bookName : '',
        this.customRequiredValidator
      ),
      year: new FormControl<number>(
        this.data ? this.data.book.year : new Date().getFullYear(),
        this.customRequiredValidator
      ),
      pagesAmount: new FormControl<number>(
        this.data ? this.data.book.pagesAmount : 0,
        this.customRequiredValidator
      ),
    });
  }
      ...
      if (controlErrors) {
        Object.keys(controlErrors).forEach((errorKey: string) => {
          this.notifyService.showMessage(
            ...
            NotifyServiceConstants.MESSAGE_TYPES.ERROR
          );
        });
      }
    });
